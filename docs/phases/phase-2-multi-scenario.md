...
- Section 1 (scenario selection) manually exercised against a scratch `.ux-audit/` with
  ...
  clean.
- Sections 3-5 (report data model, `findings-handoff.ts` generalization,
  `synthesize.ts` + `ClaudeCodeBackend.synthesizeReport`) smoke-tested end-to-end
  against the real `claude` CLI via `test/manual/synthesize-report.ts` and the fixtures
  in `test/fixtures/scenario-findings/`: `login`/`checkout` share one underlying issue
  ...
  deduplicated, and the assembled object passed `ReportSchema.parse()`. Ran in ~13-25s per
  attempt
  (no retry path exercised). Also confirmed empirically that dropping `--mcp-config`
...
- **`SynthesizeReportOptions` carries already-parsed `scenarioFindings`, not raw
  `findingsPaths`.** `synthesize.ts` reads and schema-validates each findings file once
  (via `findings-handoff.ts`'s `tryReadJson`); the backend embeds that same in-memory
  data into its prompt instead of re-reading and re-parsing the files itself. An earlier
